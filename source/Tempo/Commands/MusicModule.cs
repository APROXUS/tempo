...
namespace Tempo.Commands
{
    ...
    {
        static readonly ConcurrentDictionary<ulong, List<string>> queue = new();
        static readonly ConcurrentDictionary<ulong, bool> loop = new();
        ...
        public async Task Skip(CommandContext ctx)
        {
            if (queue[ctx.Guild.Id].Count < 2)
            {
                ...
                await ctx.RespondAsync("[Tempo]: No songs in queue...");
                return;
            }
            else
            {
        ...
        public async Task Queue(CommandContext ctx)
        {
            if (queue.TryGetValue(ctx.Guild.Id, out List<string> searches))
            {
                List<string> message = new() { "[Tempo]: The current song queue is:" };

                foreach (string search in searches) message.Add($"Search => {search}");
            ...
            {
                await ctx.RespondAsync($"[Tempo]: There is no songs queue...");
            }
        }
        #endregion
        ...
        private async Task Player(CommandContext ctx)
        {
            ...
            {
                await ctx.RespondAsync("[Tempo]: No song in queue...");
                return;
            }
        ...
        private async Task Add(CommandContext ctx, string search)
        {
            ...
            await ctx.RespondAsync($"[Tempo]: Added '{search}' song to queue...");
